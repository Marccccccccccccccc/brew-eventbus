...
public class PipelineGenerator implements Opcodes {
    ...
    private static final PipeLine EMPTY_PIPELINE = new PipeLine() {
        @Override
        public void execute(Event event) {
        }

        @Override
        public void fillFields(ErrorCallBack errorCallBack, List callees) {
        }
    };
    ...
    public static PipeLine generate(
        ...
        String className = "brewdevelopment/eventbus/generated/Pipeline_" + eventClass.getSimpleName() + "_" + ID_GEN.incrementAndGet();
        ...
        for (int i = 0; i < listeners.size(); i++) {
            ...
            mv.visitTypeInsn(CHECKCAST, Type.getInternalName(WrappedEventCaller.class));
        ...
        mv.visitVarInsn(LSTORE, 2); // Slot 2,3
        ...
        for (int i = 0; i < listeners.size(); i++) {
            if (isCancellable) {
                mv.visitVarInsn(ALOAD, 1);
                mv.visitTypeInsn(CHECKCAST, Type.getInternalName(CancellableEvent.class));
